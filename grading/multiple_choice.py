"""Multiple choice grader for BBH tasks."""
...
def _parse_answer_letter(response_text: str) -> str | None:
    """Extract a letter answer from model response after 'ANSWER:'."""
...
class MultipleChoiceGrader(Grader):
    """Grader for BBH multiple choice tasks.

    Extracts letter after 'ANSWER:' via regex, compares to sample.target.
    """
    ...
    async def grade(
        ...
        for s in samples:
            if not s.target:
                raise ValueError(f"Sample {s.index} has empty target — required for MC grading")
        ...
        for output in outputs:
            ...
            pred = _parse_answer_letter(output.response)
    ...
    def check_output(self, output: str, prompt_text: str | None = None) -> dict | None:
        pred = _parse_answer_letter(output)
        return {"pred": pred}
